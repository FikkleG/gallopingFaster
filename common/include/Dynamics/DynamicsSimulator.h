...
class DynamicsSimulator {
 ...
 {_homing.position[2] = 0.6;}
  ...
  void addCollsionShake(T mu, T rest, const Vec3<T>& pos,T period,const std::vector<Mat3<T>>& allOri)
  {
    _contact_constr->AddCollision(new CollisionShake<T>(mu,rest,pos,period,allOri));
  }
  void addCollsionFourLifters(T mu, T rest, const vector<Vec3<T>>& poss,vector<int> periods,const std::vector<vector<T>>& allHeights)
  {
      for(int i=0; i < 4; i++)
          _contact_constr->AddCollision(new CollisionLifter<T>(mu,rest,poss[i],periods[i],allHeights[i]));
  }
  /*!
