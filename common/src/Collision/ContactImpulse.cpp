...
void ContactImpulse<T>::_UpdateVelocity(DVec<T>& qdot) {
  ...
  for (size_t i(0); i < CC::_nContact; ++i) {
    ...
    if(i == 0)
    {
        std::cout<<"_penetration_recover_ratio :"<<_penetration_recover_ratio<<std::endl;
        std::cout<<"cp_local_vel :"<<cp_local_vel[2]<<std::endl;
        std::cout<<"des_vel :"<<des_vel_list_z[i]<<std::endl;
        std::cout<<"recover :"<<-_penetration_recover_ratio * CC::_cp_penetration_list[i]<<std::endl;
    }
     */
    // Contact Jacobian
    Jc_list[i] =
