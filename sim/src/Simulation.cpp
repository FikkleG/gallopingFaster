...
void Simulation::lowLevelControl()
{
  ...
  else if (_robot == RobotType::CHEETAH_3)
  {
    ...
    for (int leg = 0; leg < 4; leg++)
      for (int joint = 0; joint < 3; joint++)
      {
        _tiBoards[leg].data->q[joint] = float(_simulator->getState().q[leg * 3 + joint]);
        _tiBoards[leg].data->dq[joint] = float(_simulator->getState().qd[leg * 3 + joint]);
      }
...
void Simulation::loadTerrainFile(const std::string& terrainFileName,bool addGraphics)
{
  ...
  for (auto& key : keys)
  {
    ...
    else if(typeName == "shake")
    {
        double mu, resti, period;
        double pos[3];
        load(mu,"mu");
        load(resti,"restitution");
        load(period,"period");
        loadArray(pos, "position",3);
        addCollisonShake(mu,resti,Vec3<double>(pos),period,true);
    }
