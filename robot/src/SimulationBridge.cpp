...
void SimulationBridge::getAction()
{
    float command[12];
    char c_action[120];
    std::cin.getline(c_action,120);
    char* tmpStr = strtok(c_action,", ");
    int i = 0;
    while (tmpStr != NULL)
    {
        command[i++] = atof(tmpStr);
        tmpStr = strtok(NULL, " ");
    }
    for(int i=0;i<4;i++)
    {
        _sharedMemory().robotToSim.spiCommand.qd_des_abad[i] += command[0+3*i];
        _sharedMemory().robotToSim.spiCommand.qd_des_hip[i] += command[1+3*i];
        _sharedMemory().robotToSim.spiCommand.qd_des_knee[i] += command[2+3*i];
    }

}

void SimulationBridge::publishState()
{
    std::cout.seekp(0);
    std::cout<<"pubilsh--------start\n"<<_sharedMemory().simToRobot.cheaterState.omegaBody<<"\nend"<<std::endl;
    std::cout.flush();
}