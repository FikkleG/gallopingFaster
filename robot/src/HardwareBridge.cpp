...
typedef struct
{
    double DirectionFeedback;
    double DirectionDestination;
}ClientMessage;
...
void MiniCheetahHardwareBridge::initHardware() {
  ...
  if(_microstrainInit)
  printf("!!!!!!!!!!!!!!!!!!_microstrainInit True\n");
  else
  printf("!!!!!!!!!!!!!!!!!!_microstrainInit False\n");
}
...
{
#ifdef DISPLAY_INIT
    cout<<"\t begin to make the thread to TCP-Thread "<<endl;
	cout<<"\t TCP: creat Socket"<<endl;
#endif
...
#ifdef DISPLAY_INIT
    cout<<"\t TCP: initialize Socket attribution"<<endl;
#endif
...
#ifdef DISPLAY_INIT
    cout<<"\t TCP: bind Socket with attribution"<<endl;
#endif
...
void* TCPThread(void *arg )
{
    ...
    sleep(1);
#ifdef DISPLAY_INIT
    cout<<"TCP thread begin running "<<endl;
#endif
    openTCP();
#ifdef DISPLAY_INIT
    cout<<"TCP thread come to while(1) "<<endl;
#endif
    while(1)
    {
        if(!connected)
        {
...
#ifdef DISPLAY_INIT
            cout<<"\t Server Socket wait to be connected ......"<<endl;
#endif
            ...
            connected = true;
#ifdef DISPLAY_INIT
            cout<<"\t Server Socket connected with "<<inet_ntoa(cliaddr.sin_addr) <<endl;
#endif
            //	printf("Server get connection from %s\n",inet_ntoa(cliaddr.sin_addr));
        }
        else
        {
            if((ret=recv(clifd,&tcpclientmsg,sizeof(TCPClientMessage),0))==-1) //TCP
            {
                printf("Read Error:%s\n",strerror(errno));
                connected=false;
                continue;
            ...
            if(ret==sizeof(TCPClientMessage))
            {
