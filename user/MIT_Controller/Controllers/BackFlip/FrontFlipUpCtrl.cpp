...
#include "FrontFlipUpCtrl.h"

template <typename T>
FrontFlipCtrl<T>::FrontFlipCtrl(DataReader* data_reader,float _dt) : DataReadCtrl<T>(data_reader, _dt) {}

template <typename T>
FrontFlipCtrl<T>::~FrontFlipCtrl() {}

template <typename T>
void FrontFlipCtrl<T>::OneStep(float _curr_time, bool b_preparation, LegControllerCommand<T>* command) {
...
template <typename T>
void FrontFlipCtrl<T>::_update_joint_command() {
...
//          current_step[4],current_step[5],current_step[6] );
    ...
    // CONTROL LEG_CLEARANCE_ITERATION_FRONT 大于240个迭代小于600个
...
template class FrontFlipCtrl<double>;
template class FrontFlipCtrl<float>;
