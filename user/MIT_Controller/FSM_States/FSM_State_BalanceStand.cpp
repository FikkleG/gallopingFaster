...
void FSM_State_BalanceStand<T>::BalanceStandStep()
{
  ...
  if(this->_data->controlParameters->use_rc)
  {
    ...
    // Orientation
    _wbc_data->pBody_RPY_des[0] = rc_cmd->rpy_des[0]*0.9;
    _wbc_data->pBody_RPY_des[1] = rc_cmd->rpy_des[1]*0.46;
    _wbc_data->pBody_RPY_des[2] -= rc_cmd->rpy_des[2]*0.8;

    // Height
//    _wbc_data->pBody_des[2] += 0.12 * rc_cmd->height_variation;
if(rc_cmd->height_variation>0)
    _wbc_data->pBody_des[2] += 0.1 * rc_cmd->height_variation;
else
    _wbc_data->pBody_des[2] += 0.28 * rc_cmd->height_variation;
  }
  else
  {
    // Orientation
