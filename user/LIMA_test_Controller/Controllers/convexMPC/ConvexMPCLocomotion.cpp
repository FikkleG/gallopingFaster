...
ConvexMPCLocomotion::ConvexMPCLocomotion(float _dt, int _iterations_between_mpc, LIMAuserParameters* parameters) :
  iterationsBetweenMPC(_iterations_between_mpc),
  horizonLength(10),
  dt(_dt),
  trotting(horizonLength, Vec4<int>(0,5,5,0), Vec4<int>(5,5,5,5),"Trotting"),
  slowtrotting(int(horizonLength*1.2), Vec4<int>(0,6,6,0), Vec4<int>(6,6,6,6),"SlowTrotting"),
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    footSwingTrajectories[i].setHeight(step_height);//.125);
     ...
     if(i<2)
      ...
      {
          if(i==0)
              offset(1)=-0.1*(1-fabs(v_des_robot[0])/2.0);
          else if(i==1)
              offset(1)=0.1*(1-fabs(v_des_robot[0])/2.0);
      }
...
//    updateMPCIfNeeded(gait, mpcTable, data, omniMode, 10);
...
void ConvexMPCLocomotion::solveDenseMPC(int *mpcTable, ControlFSMData<float> &data) {
  auto seResult = data._stateEstimator->getResult();
  float Q[12] = {3.0, 2.0, 10.0, 2, 2, 100, 0, 0, 0.3, 5, 5, 2}; // trot
